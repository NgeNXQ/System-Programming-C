...
static struct cdev labwork5_device;
...
static int __init mytimemodule_init(void) 
{
	...
	if((result = alloc_chrdev_region(&device_number, 0, 1, "labwork5_device")) < 0)
	{
		print_message("Cannot allocate major number\n");
		return result;
	}
	
	cdev_init(&labwork5_device, &fops);
	print_message("Major = %d Minor = %d\n", MAJOR(device_number), MINOR(device_number));

	if((result = cdev_add(&labwork5_device, device_number, 1)) < 0)
	{
	...
	device = device_create(device_class, NULL, device_number, NULL, "labwork5_device");
